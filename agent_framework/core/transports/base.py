...
class TransportRegistry:
    """Registry for provider transports.

    Maintains a mapping of api_mode -> ProviderTransport class.
    """

    def __init__(self):
        self._transports: Dict[str, type] = {}

    def register(self, api_mode: str, transport_class: type) -> None:
        """Register a transport class for an api_mode."""
        if not issubclass(transport_class, ProviderTransport):
            raise TypeError(f"{transport_class} must be a subclass of ProviderTransport")
        self._transports[api_mode] = transport_class

    def get(self, api_mode: str) -> ProviderTransport:
        """Get a transport instance by api_mode."""
        if api_mode not in self._transports:
            raise ValueError(f"No transport registered for api_mode: {api_mode}")
        return self._transports[api_mode]()

    def list_modes(self) -> List[str]:
        """List all registered transport api_modes."""
        return list(self._transports.keys())

    def has(self, api_mode: str) -> bool:
        """Check if a transport is registered for api_mode."""
        return api_mode in self._transports
...
def register_transport(api_mode: str, transport_class: type) -> None:
    """Register a transport class for an api_mode."""
    _registry.register(api_mode, transport_class)
...
def has_transport(api_mode: str) -> bool:
    """Check if a transport is registered for api_mode."""
    return _registry.has(api_mode)
